...
class AEPredNet(nn.Module):
    ...
    def __init__(self, input_size=512, latent_size=128, lr=1e-3, decay_rate=0.999, dtype=torch.float, p_drop=0.1,
        ...
        # Encoder
        self.fc_e1 = nn.Linear(in_features=input_size, out_features=input_size // 2)
        ...
        # Decoder
        self.fc_d1 = nn.Linear(in_features=latent_size, out_features=input_size // 2)
        self.fc_d2 = nn.Linear(in_features=input_size // 2, out_features=input_size // 4)
    ...
    def forward(self, inputs, predict=True):

        enc = self.drop(self.act(self.fc_e1(inputs)))
        enc = self.drop(self.act(self.fc_e2(enc)))
        # enc = self.fc_e3(enc)
        enc = self.act(self.fc_e3(enc))
        latent = enc

        dec = self.drop(self.act(self.fc_d1(enc)))
        dec = self.drop(self.act(self.fc_d2(dec)))
        dec = self.drop(self.act(self.fc_d3(dec)))
    ...
    def train_step_ae(self, inputs, targets=None, alpha=1, predict=True):
        self.opt.zero_grad()
        self.train()
        # print(input.shape)
        outputs = self(inputs)
        out = outputs[0]
        loss1 = self.rec_loss(inputs, out)
        if predict:
            pred = outputs[2]
