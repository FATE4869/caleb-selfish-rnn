...
def jac_list(fcon, model, h_t, x_t):
    ...
    for r in range(l):
        a = rnn_jac(model.rnn_layer.all_weights, h_t[:, r].unsqueeze(0), x_t[:, r].unsqueeze(1),
                    bias=fcon.model.rnn_atts['bias'])
        # print(a.shape)
...
def jac_product(fcon, J_list, t, s):
    J_prod = torch.eye(fcon.model.rnn_atts['hidden_size']).unsqueeze(0).unsqueeze(0).repeat(s - t + 1, le_batch_size, 1,
                                                                                            1)
    ...
    for p in params[:1]:
        ...
        for epoch in range(1, max_epoch + 1, save_interval):
            ...
            for it in range(in_epoch_saves + 1):
                ...
                J_list = jac_list(fcon, model, h_in.transpose(0, 1),
                                  x_list)  # List of all the Jacobians for the sequences
                ...
                r_prod = torch.zeros(
                    (seq_length + 1, seq_length + 1, le_batch_size, hidden_size, hidden_size)).cpu()  # rvalues
                q_prod = torch.zeros(
                    (seq_length + 1, seq_length + 1, le_batch_size, hidden_size, hidden_size)).cpu()  # full Q-product
                j_prod = torch.zeros(
                    (seq_length + 1, seq_length + 1, le_batch_size, hidden_size, hidden_size)).cpu()  # Direct Jacobians
                ...
                if qs:
                                ...
                                for t in range(0, seq_length):
                                    ...
                                    for s in range(t, seq_length + 1):
                                        ...
                                        if s == t:  # when s = t, the product is empty and is replaced with the identity
                                            q_prod[t, s] = torch.eye(fcon.model.rnn_atts['hidden_size']).unsqueeze(
                                                0).repeat(le_batch_size, 1, 1)
                    ...
                    if not os.path.exists(qgrad_name) or overwrite:
                        ...
                        if os.path.exists(qp_fname) and not overwrite:
                            ...
                            # print(f'rvals shape: {rvals.shape}')
                            for t in range(1, seq_length):
                                # print(f't = {t}')
                                r_prod[t, t] = torch.eye(hidden_size).unsqueeze(0).repeat(le_batch_size, 1, 1)
                                q_prod[t, t] = torch.eye(hidden_size).unsqueeze(0).repeat(le_batch_size, 1, 1)
                                for s in range(t + 1, seq_length + 1):
                                    # print(f'rvals shape: {rvals[s-1].shape}')
                                    # r_prod[t, s] = (rvals[s-1].transpose(-1, -2))@r_prod[t, s-1]
                                    r_prod[t, s] = torch.matmul(rvals[s - 1], r_prod[t, s - 1])
                                    # q_prod[t, s] = (qvec_list[:, t]@r_prod[t, s]@qvec_list[:,s].transpose(-2,-1)).transpose(-2,-1)
                                    q_prod[t, s] = (qvec_list[:, s] @ r_prod[t, s] @ qvec_list[:, t].transpose(-2, -1))
                ...
                for q in range(hidden_size):
                    ...
                    diff_norms.append(
                        torch.mean(torch.linalg.norm(gradV_J.to(device) - gradQ.to(device), dim=(-2, -1))).detach())
